...
vec4 sample_catmull_roll(in sampler2D image, in vec2 uvs) {
    const ivec2 resolution = textureSize(image, 0);
    const vec2 sample_position = uvs * resolution;
    const vec2 uvs_1 = floor(sample_position - 0.5f) + 0.5f;
...
void main() {
    if (ignore_history == 1) {
        o_pixel = vec4(texture(u_final_color, i_uvs).rgb, 1.0);
    } else {
        ...
        vec2 closest_depth_pixel = vec2(0);
        const ivec2 resolution = textureSize(u_final_color, 0);

        for (int x = -1; x <= 1; x++) {
            for (int y = -1; y <= 1; y++) {
                vec2 pixel_position = i_uvs + (ivec2(x, y) / vec2(resolution));
                pixel_position = clamp(pixel_position, 0.0, 1.0);

                const vec3 neighbor = max(vec3(0), texture(u_final_color, i_uvs).rgb);
                ...
                const float current_depth = texture(u_scene_depth, pixel_position).r;
        ...
        const vec2 motion_vector = texture(u_taa_velocity, closest_depth_pixel).xy * vec2(0.5, -0.5);
        ...
        vec3 history_sample = sample_catmull_roll(u_taa_history, history_uv).rgb;
        ...
        history_sample = compute_aabb_clip(
            minc, maxc,
            vec4(clamp(history_sample, neighborhood_min, neighborhood_max), 1.0),
            vec4(history_sample, 1.0)).rgb;
