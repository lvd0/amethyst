...
mat4 compute_jittered_proj_view() {
    mat4 projection = camera.projection;
    projection[2][0] += jitter_offset.x;
    projection[2][1] += jitter_offset.y;
...
void main() {
    ...
    const mat4 jittered_proj_view = compute_jittered_proj_view();
    const vec4 clip_vertex_0 = jittered_proj_view * model * vec4(raw_vertex_0, 1.0);
    const vec4 clip_vertex_1 = jittered_proj_view * model * vec4(raw_vertex_1, 1.0);
    const vec4 clip_vertex_2 = jittered_proj_view * model * vec4(raw_vertex_2, 1.0);
